...
- `src/_data/site.json` — site-wide settings not tied to a specific content list. Currently just `recordsFormUrl` (the Google Form link for the Announcements page's "Submit a Record" box) — empty string until the user shares the real form link, at which point the CTA box switches from its "coming soon" state to a live button automatically (no template change needed, just fill in the value).
...
3. **Records Google Form link not set yet** (`src/_data/site.json`'s `recordsFormUrl` is `""`). User said they'll share the form URL soon — once given, set that one field and rebuild; the Announcements page's "Submit a Record" box picks it up automatically.
4. **Governance pages are stubs** — real content exists in draft form in `Documents/Vyzkaris/VYZKARIS COVENANT — *.md` (Main Idea, Structure, Legion System, Territory, Diplomacy, Judiciary, Recruitment System, State-Enterprise System) and should eventually be adapted into `src/governance/*.md`.
5. **Resources and Gallery have no real entries yet** (`src/_data/resources.json` and `gallery.json` are both `[]`). Add entries as the user has real Drive links / images to publish; gallery images themselves go in `src/assets/images/gallery/`.
6. **No custom domain configured** — site lives at the default `github.io` project-pages URL. Not in scope unless asked.
